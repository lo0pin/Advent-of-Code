# 🎄 Advent of Code – Progress Tracker

A collection of my solutions for **Advent of Code**, implemented day by day throughout December.  
Each day brings **two puzzles**: a warm-up and a twist.  
This repository documents my progress and provides clean, reproducible code solutions.

---

## ❄️ Short Description  
Advent of Code is a festive coding event where small daily challenges form a growing holiday story.  
Each task tests logic, creativity, and problem-solving — wrapped in a delightful Christmas atmosphere.

---

## 🎁 Progress Overview (2025)

<details>
  <summary>AoC 2025 Progress</summary>

| Day | Part 1 solved | Part 2 solved |
|-----|--------|--------|
| 001 | [x] | [x] |
| 002 | [x] | [x] |
| 003 | [x] | [x] |
| 004 | [x] | [x] |
| 005 | [x] | [x] |
| 006 | [x] | [ ] |
| 007 | [x] | [ ] |
| 008 | [x] | [ ] |
| 009 | [x] | [ ] |
| 010 | [x] | [ ] |
| 011 | [x] | [ ] |
| 012 | [ ] | [ ] |

</details>


<details>
  <summary>AoC 2025 optimized code Progress</summary>

| Day | Part 1 optimized | Part 2 optimized |
|-----|--------|--------|
| 001 | [x] | [x] |
| 002 | [x] | [x] |
| 003 | [ ] | [ ] |
| 004 | [ ] | [ ] |
| 005 | [ ] | [ ] |
| 006 | [ ] | [ ] |
| 007 | [ ] | [ ] |
| 008 | [ ] | [ ] |
| 009 | [ ] | [ ] |
| 010 | [ ] | [ ] |
| 011 | [x] | [ ] |
| 012 | [ ] | [ ] |


</details>


<details>
  <summary>AoC 2025 best and smartest solutions review</summary>

| Day | Part 1 optimized | Part 2 optimized |
|-----|--------|--------|
| 001 | [ ] | [ ] |
| 002 | [ ] | [ ] |
| 003 | [ ] | [ ] |
| 004 | [ ] | [ ] |
| 005 | [ ] | [ ] |
| 006 | [ ] | [ ] |
| 007 | [ ] | [ ] |
| 008 | [ ] | [ ] |
| 009 | [ ] | [ ] |
| 010 | [ ] | [ ] |
| 011 | [ ] | [ ] |
| 012 | [ ] | [ ] |


</details>


---

## 🎄 Repository Structure

```
/2025/
├─ 001/
│ ├─ 001Input.py # Raw puzzle input / preprocessing
│ ├─ 001task.md # Puzzle description
│ ├─ 001partOne.py # My Part 1 solution
│ ├─ 001partTwo.py # My Part 2 solution
│ ├─ 001partOne_bestSolution.py # Optimized / reference Part 1
│ ├─ 001partTwo_bestSolution.py # Optimized / reference Part 2
│ └─ README.md # optional notes
│
├─ 002/
│ └─ ... # same structure for each day
│
└─ utils/ # optional helpers, shared tools
```


**Naming convention:**  
`0xx` = day number (001–025).  
Each folder is fully self-contained and reproducible.

---

## ⭐ Thanks  
Thanks to **Eric Wastl** for creating Advent of Code  
and to the community for inspiration, clever ideas, and algorithmic holiday spirit.

Merry coding — and may every puzzle fall before your sleigh! 🎅✨
